...
for iecClient (i.e. master or controlling station) and tcpServer (i.e. slave
or controlled station) applications.

You can use j60870 to program your individual iecClient or tcpServer
applications. A simple console iecClient is part of the library. You can
...
console iecClient and the sample tcpServer which can be found here:
cli-app/src/main/java/org/mamotec/j60870/app/ . These applications in
combination with the javadoc should satisfy most of your documentation
needs.

Here is a short summary of the steps to get a iecClient running:

* Create and configure an instance of ClientConnectionBuilder.

* Connect to the tcpServer using ClientConnectionBuilder.build() which
  returns the tcpConnection. The iecClient is now connected to the tcpServer
  via TCP/IP.
...
* *Monitor direction* - direction from tcpServer to iecClient

* *Control direction* - direction from iecClient to tcpServer
...
* *STARTDT ACT* - Start data tranfer message. Needs to be sent by the
   iecClient before information messages may be exchanged between iecClient
   and tcpServer.
...
=== In iecClient tcpConnection implemented commands
