...
for iecClient (i.e. master or controlling station) and server (i.e. slave
or controlled station) applications.

You can use j60870 to program your individual iecClient or server
...
console iecClient and the sample server which can be found here:
...
* Connect to the server using ClientConnectionBuilder.build() which
  returns the connection. The iecClient is now connected to the server
  via TCP/IP.
...
* *Monitor direction* - direction from server to iecClient

* *Control direction* - direction from iecClient to server
...
=== In iecClient connection implemented commands
