	...
	public static InformationObject decode(ExtendedDataInputStream is, ASduType aSduType, int numberOfSequenceElements, int ioaFieldLength) throws IOException {
		...
		case M_EP_TF_1:
			informationElements = new InformationElement[][] {
					{ new IeProtectionOutputCircuitInformation(is), new IeProtectionQuality(is), new IeTime16(is), IeTime56.decode(is) } };
			break;
		// 45
		case C_SC_NA_1:
		...
		case F_SR_NA_1:
			informationElements = new InformationElement[][] {
					{ IeNameOfFile.decode(is), IeNameOfSection.decode(is), IeLengthOfFileOrSection.decode(is), IeSectionReadyQualifier.decode(is) } };
			break;
		// 122
		case F_SC_NA_1:
		...
		// 123
		case F_LS_NA_1:
			return new InformationObject(informationObjectAddress, IeNameOfFile.decode(is), IeNameOfSection.decode(is), IeLastSectionOrSegmentQualifier.decode(is),
					IeChecksum.decode(is));
		// 124
		case F_AF_NA_1:
		...
		case F_DR_TA_1:
			...
			for (int i = 0; i < numberOfSequenceElements; i++) {
				informationElements[i] = valuesAsArray(IeNameOfFile.decode(is), IeLengthOfFileOrSection.decode(is), IeStatusOfFile.decode(is), IeTime56.decode(is));
			}
			break;
		// 127
		case F_SC_NB_1:
		...
		default:
			throw new IOException("Unable to parse Information Object because of unknown Type Identification: " + aSduType);
		}
